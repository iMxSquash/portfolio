...
- **Réglages Système** : même chrome unifié que Finder/Notes (sidebar plate pleine hauteur avec traffic lights, champ de recherche des panneaux, toolbar précédent/suivant). Panneaux enregistrés dans `src/lib/settings-panes.ts` (même principe que `finder.ts`). Store persistant `useSettingsStore` ; réutilise `useThemeStore`/`useWallpaperStore` pour le thème et le fond d'écran plutôt que de dupliquer cet état. Les réglages Liquid Glass passent uniquement par `resolveGlassConfig` (`src/lib/glass-settings.ts`), jamais par une config recopiée dans un panneau (voir CLAUDE.md).
